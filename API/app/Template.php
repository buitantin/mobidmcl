...
class Template extends Model {
	...
	public function getFilter_Parent($id_cate){
		return Cache::remember("get_template_filter_parent_".$id_cate,40,function() use($id_cate){
	...
	*/
	public static function getFilter_Child($id_cate){
		...
		return Cache::remember("get_filter_template_child".$id_cate,60,function() use($id_cate){
	...
	public function getTotal_Filter($id_element_child,$id_cate,$val){
		$c=md5($id_element_child.$id_cate.$val);
		return Cache::remember("get_template_total_filter_".$c,10,function() use($id_element_child,$id_cate,$val){
	...
	public static function Compare_One($id,$supplier){
		return Cache::remember("get_template_compare_one_".$id.$supplier,10,function() use($id,$supplier){
				if(!empty($id)){
	...
	public static function Compare_Cate($id_cate,$id_product=null){
		return Cache::remember("get_template_compare_cate_".$id_cate.$id_product,10,function() use($id_cate,$id_product){
				$sql=(empty($id_product) ? "b.is_type='0' AND c.is_compare='1' " : "b.is_type='0' AND c.is_compare='1' AND d.cid_product=$id_product");

				$sql=$sql." AND x.cid_product=$id_product";
				...
				return DB::table("comp_template AS a")
							...
							->join("comp_elemt_product AS x",function($join){
								$join->on("x.cid_element","=","c.id");
							})
							->selectRaw("a.id,b.is_type,c.name,x.val")
	...
	public static function get_Product($id,$element){
		...
		return Cache::remember("get_template_get_product_".$id.$element,10,function() use($id,$element){
	...
	public function Compare($array){
		$c=md5($array);
		return Cache::remember("get_template_compare1_".$c,10,function() use($array){
				$sql="";
				$supplier="";
				foreach ($array as $a){
					$sql .= " b.id={$a['id']} OR";
